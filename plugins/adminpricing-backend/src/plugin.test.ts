...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [adminpricingPlugin],
    });

    await request(server).get('/api/adminpricing/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/adminpricing/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/adminpricing/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/adminpricing/todos/${createdTodoItem.id}`)
